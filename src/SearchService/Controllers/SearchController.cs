...
public class SearchController : ControllerBase
{
    ...
    public async Task<ActionResult<List<Item>>> SearchItem([FromQuery]SearchParams searchParams)
    {
        ...
        {
            "condition" => query.Sort(x => x.Descending(a => a.Condition)),
        ...
        {
            query.Match(x => x.Seller == searchParams.Seller).SortByTextScore();
        }

        if (!string.IsNullOrEmpty(searchParams.Winner))
        {
            query.Match(x => x.Winner == searchParams.Winner).SortByTextScore();
        }
